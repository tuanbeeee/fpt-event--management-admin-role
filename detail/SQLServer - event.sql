USE [master]
GO
CREATE DATABASE FPT_EVENT_MANAGEMENT
GO
USE FPT_EVENT_MANAGEMENT
GO
...
CREATE TABLE tblUsers (
	...
	email varchar(50),
	status bit not null,
	typeID varchar(10) foreign key references tblUserTypes (typeID),
...
CREATE TABLE tblOrgPage (
	...
	imgUrl varchar(max),
	email varchar(100),
	statusTypeID varchar(20) foreign key references tblStatusType(statusTypeID)
)
go
...
(	
	...
	orgID varchar(20) foreign key references tblOrgPage (orgID),
	status bit,
	orgFollowerID int identity(1,1) primary key
)
go
...
CREATE TABLE tblEventPost (
	...
	content ntext,
	title ntext,
	location int foreign key references tblLocation(locationID),
	imgUrl varchar(max),
	eventTypeID int foreign key references tblEventType(eventTypeID),
	numberOfView int,
	speaker nvarchar(100),
	approvalDes varchar(max),
	summary nvarchar(max) not null
)
go
CREATE TABLE tblUserNotification
(
	userNoti int identity(1,1) primary key,
	userID varchar(50) foreign key references tblUsers (userID),
	eventID varchar(20) foreign key references tblEventPost (eventID),
...
(
	userFeedback int primary key IDENTITY(1,1),
	userID varchar(50) foreign key references tblUsers (userID),
	eventID varchar(20) foreign key references tblEventPost (eventID),
	content ntext
)
...
CREATE TABLE tblManagers (
	managerID varchar(50) foreign key references tblUsers (userID),
	orgID varchar(20) foreign key references tblOrgPage (orgID),
...
insert into tblBlog (blogID, orgID, status, title, createDate, content, imgUrl, numberOfView, summary) values ('BLG6', 'SCT', '1', N'REVIEW NHẸ CÁC NGÔN NGỮ, CÔNG NGHỆ, FRAMEWORK MÀ MÌNH ĐÃ VÀ ĐANG DÙNG KIẾM CƠM – PHẦN CUỐI', '1/7/2022', N'Hồi đó mình cũng thấy ghét, lại nghe nói code Angular rườm rà, build nặng nên cũng không thèm học.
